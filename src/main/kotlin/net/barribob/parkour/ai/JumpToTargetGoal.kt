...
class JumpToTargetGoal(private val entity: MobEntity) : Goal() {
    ...
    private fun getMobJumpAbilities(): Triple<Double, Int, Double>  {
        val jumpYVel = ModUtils.getJumpVelocity(entity.world, entity) // Maximum y velocity for a jump. Used in determining if an entity can make a jump
    ...
    private fun getJumpLength(actorPos: Vec3d, targetDirection: Vec3d): Pair<Vec3d, Pair<Double, Double>>? {
        ...
        for ((x, y) in jumpOffsets) {
            ...
            val cornerPos = ModUtils.findClosestCorner(offsetPos, blockShape, 16)?.add(walkablePos.asVec3d()) ?: continue
    ...
    private fun getNode(pos: BlockPos): BlockType {
        return ModUtils.getBlockType(entity.world, pos, 2)
    }
}
